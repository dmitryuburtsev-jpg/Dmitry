...
for r in [(18, 19.2, 1, 12.8), (18.9, 7.6, 1.2, 18.9), (20.1, 7.6, 6.2, 1.2), (9.7, 12.3, 9.2, 1.2), (9.7, 3.6, 1.2, 8.7), (3.4, 3.6, 6.3, 1.2), (15.6, 13.5, 1.4, 0.9)]:
...
pos = {0: (595.0, 360.0), 7: (748.0, 566.0), 11: (795.0, 545.0)}
for n, (x, y) in pos.items():
    mk[2*n] = mv(mk[2*n], x, y); mk[2*n+1] = mv(mk[2*n+1], x, y)
out += mk
s = s[:i] + head + ''.join(out) + '</svg>' + s[j:]
tot = int(open('site_total.txt').read())
for a, b in [('Дом 85 м²: вход и крыльцо со стороны калитки, терраса в сад', 'Дом 6×9, 2 этажа, 99,9 м²: терраса в сад, вход со стороны калитки'),
