...
V1_UNDER = [(HX1, 21.0), (15.8, 21.0), (15.8, 25.1)]   # в грунте под домом на −1,8 (футляр — до x = 15,95), подъём в котельной у ГА
...
# магистраль к беседке и хозблоку от TRX до x = 9,0 считается один раз (раньше учитывался только участок 10,3 → 9,0)
EL_TRENCH = (32.0 - YG) + (YV - 10.4) + L(EL["баня"]) + (TRX - 9.0) + L(EL["хозблок"][1:]) + L(EL["ЛОС"][1:])
